...
- [ ] Capabilities/Initialize/Shutdown/Lifecycle 约定与校验
- [x] JSON 序列化配置（大小写策略、忽略空值、数字/时间处理）
- [ ] 消息校验（必填字段、Schema 校验钩子）

### 传输层（抽象 ITransport）
- [x] Stdio 传输（默认，支持 FastMCP 生态）
- [ ] WebSocket 传输（可选，便于远程/浏览器桥接）
- [ ] 命名管道（Windows 可选）
- [ ] 背压与缓冲策略、取消/超时、心跳与重连

### 服务器（AxolotlMCP.Server）
- [ ] HostBuilder 集成（DI/Logging/Options）
- [ ] 消息路由器（请求/通知 → 处理器）
...
### 配置（Options）
- [ ] Options 模型（传输、并发、超时、重试、日志级别）
- [ ] appsettings.json / 环境变量绑定
- [ ] 示例配置同步到 `samples/`

### 可观测性（Observability）
- [ ] 结构化日志（请求/响应/错误摘要）
- [ ] 指标（请求时延、QPS、成功率、异常率）
- [ ] OpenTelemetry Tracing（可选，跨进程链路）
...
### 文档与示例
- [ ] README 快速开始（Server/Client 使用）
- [ ] 按传输分类的示例：stdio / WebSocket
- [ ] 工具/资源/提示完整示例
- [ ] 架构/扩展点文档（docs/）
